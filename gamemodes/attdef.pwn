...
public OnPlayerConnect(playerid)
{
	...
	SendClientMessage(playerid, -1, sprintf("                    "COL_PRIM"Welcome to {FFFFFF}%s %.2f (r)", "Comoter Attack Defend));
	#else
	SendClientMessage(playerid, -1, sprintf("                       "COL_PRIM"Welcome to {FFFFFF}%s %.2f", "Comoter Attack Defend"));
	#endif

	SendClientMessage(playerid, -1, "CMD DASAR : /help, /cmds, /acmds, /settings, /cmdhelp, /matchtips");
	SendClientMessage(playerid, -1, ""COL_PRIM"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
...
public ServerOnPlayerDeath(playerid, killerid, reason)
{
	...
	else
	{
		...
		if(Player[playerid][Playing] == true && Player[killerid][Playing] == true && AllowStartBase != false)
		{
			...
			new str[150], dcstring[200];
			format(dcstring, sizeof(dcstring), "***%s*** telah terbunuh oleh ***%s***\n\nPlayer: %s(%d)\nKiller: %s(%d)\nKilled With: %s\nDistance: %.1f ft", Player[playerid][Name], Player[killerid][Name], Player[playerid][Name], playerid, Player[killerid][Name], killerid, WeaponNames[reason], GetDistanceBetweenPlayers(killerid, playerid));
			SendDiscordEmbedMessage(0, "Death", dcstring, "Kill Logs");
...
public OnPlayerText(playerid, text[])
{
    ...
    if(text[0] == '!')
	{
	    new ChatColor, dstring[144];
		if(Player[playerid][Team] == ATTACKER || Player[playerid][Team] == ATTACKER_SUB)
		{
			format(dstring, sizeof dstring, "```%s Team Chat | %s (%d) | %s```", TeamName[Player[playerid][Team]], Player[playerid][Name], playerid, text[1]);
			SendDiscordMessage(8, dstring);
		}
		else if(Player[playerid][Team] == DEFENDER || Player[playerid][Team] == DEFENDER_SUB)
		{
			format(dstring, sizeof dstring, "```%s Team Chat | %s (%d) | %s```", TeamName[Player[playerid][Team]], Player[playerid][Name], playerid, text[1]);
			SendDiscordMessage(9, dstring);
		}
		else if (Player[playerid][Team] == REFEREE)
		{
			format(dstring, sizeof dstring, "```%s Team Chat | %s (%d) | %s```", TeamName[Player[playerid][Team]], Player[playerid][Name], playerid, text[1]);
			SendDiscordMessage(10, dstring);
		}
	    ...
	    new ChatString[144];
		format(ChatString,sizeof(ChatString),"%s Team Chat | %s (%d) | %s", TeamName[Player[playerid][Team]], Player[playerid][Name], playerid, text[1]);
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
    ...
    SendClientMessage(playerid, -1, "{FFFFFF}Gunakan sync untuk terjun.");
	Player[playerid][LastEnterVehicleAttempt] = GetTickCount() + 3000;
	return 1;
}
	...
	if(dialogid == DIALOG_STARTMATCH)
	{
		if(response)
		{
		    ...
		    switch(listitem)
		    {
		        case 0:
		        {
		            CallLocalFunction("OnPlayerCommandText", "ds", playerid, "/rb"); // attack defend
		        }
		        case 1:
		        {
		            CallLocalFunction("OnPlayerCommandText", "ds", playerid, "/ra"); // team deathmatch
		        }
		        case 2:
		        {
		            CallLocalFunction("OnPlayerCommandText", "ds", playerid, "/start last"); //start last a/d atau tdm
		        }
		    }
		}
	}
...
YCMD:netcheckall(playerid, params[], help)
{
	...
	if(help)
	{
		SendCommandHelpMessage(playerid, "Netchecking all players in server");
		return 1;
	}

	new iString[130], str[256];
	foreach(new pID : Player)
	{
		if(toggle == 0) {
			...
			format(str, sizeof(str), "{FFFFFF}%s "COL_PRIM"has disabled Net-Check to all players", Player[playerid][Name]);
		} else {
			Player[pID][NetCheck] = 1;
			Player[pID][FPSCheck] = 1;
			Player[pID][PingCheck] = 1;
			Player[pID][PLCheck] = 1;
			format(str, sizeof(str), "{FFFFFF}%s "COL_PRIM"has enabled Net-Check to all players", Player[playerid][Name]);
		}
...
YCMD:netchecklog(playerid, params[], help)
{
	new str[256];
	format(str, sizeof str, "Netcheck status: %d", Player[playerid][NetCheck]);
...
YCMD:para(playerid, params[], help)
{
	...
	RemovePlayerWeapon(playerid, 46);
    SendClientMessage(playerid, -1, "{FFFFFF}Gunakan shift + kanan pada mouse untuk sync.");
	return 1;
}
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if(newkeys == 160 && GetPlayerVehicleID(playerid) == 0)
	{
		...
		if(synctimer[playerid] == 1) return SendErrorMessage(playerid, "Can't sync during cooldown");
		SyncPlayer(playerid);
	    return 1;
	}
	...
	if(Current == -1)
	{
		...
		{
			//EnableMatchInterface(playerid);
			ShowPlayerDialog(playerid, DIALOG_STARTMATCH, DIALOG_STYLE_LIST, "SA:MP Master League", "Attack Defend\nTeam Deathmatch\nStart Last", "Pilih", "Batal");
			return 1;
		}
